...
export default function Aside(){
  ...
  const hovers = 'cursor-pointer w-full flex items-center gap-2 py-3 px-4 border-b-slate-50 border-b hover:bg-slate-200  hover:scale-105 transition hover:text-cyan-400'
  return (
    <div className='asse flex overflow-hidden  rounded-md w-[60%] right-0  h-[max-content]  z-20 lg:hidden lg:text-3xl text-2xl top-16 absolute'>
       <ul className='w-[100%]  text-slate-200 flex items-start flex-col'>
          ...
          <Link to='/about'><li className={`${hovers}`}>
            <BiInfoCircle/> About<span className='invisible'>hfhfhfhfhfhgkgkkkhjkhkhkhhkhkhkhihi</span>
            </li></Link>
          ...
          className={`cursor-pointer py-2 ${hovers} border-b-slate-50 border-b transition hover:text-cyan-400 hover:scale-105 relative  flex items-center`}>
           <Fa6SolidDiagramProject/> Projects
            {projectA?<MaterialSymbolsKeyboardArrowUp/>:<MaterialSymbolsKeyboardArrowDown/>}
            </li>
            {projectA?<><Link to='/project/OWN'><li className={hovers}><TablerShieldCode/>OWN<span className='invisible'>hfhfhfhfhfhgkgkkkhjkhkhkhhkhkhkhihi</span></li></Link>
      <Link to='/project/clones'><li className={hovers}><Fa6RegularClone/>clones<span className='invisible'>hfhfhfhfhfhgkgkkkhjkhkhkhhkhkhkhihi</span></li></Link>
      <Link to="/project/API's"><li className={hovers}><CarbonApi1/>API's<span className='invisible'>hfhfhfhfhfhgkgkkkhjkhkhkhhkhkhkhihi</span></li></Link>
      <Link to='/project/when-bored'><li className={hovers}><MdiLightSleep/>Bored<span className='invisible'>hfhfhfhfhfhgkgkkkhjkhkhkhhkhkhkhihi</span></li></Link>
      <Link to='/project/custom'><li className={hovers}><GridiconsCustomize/>Custom<span className='invisible'>hfhfhfhfhfhgkgkkkhjkhkhkhhkhkhkhihi</span></li></Link></>:null}
          <li onClick={()=>{setSkillsA(!skillsA); setProjectA(false)}} 
          className={`cursor-pointer py-2 ${hovers} border-b-slate-50 border-b transition hover:text-cyan-400 hover:scale-105 relative  flex items-center`}>
           <CarbonSkillLevelIntermediate/> Skills
            { skillsA?<MaterialSymbolsKeyboardArrowUp/>:<MaterialSymbolsKeyboardArrowDown/>}
            </li>
